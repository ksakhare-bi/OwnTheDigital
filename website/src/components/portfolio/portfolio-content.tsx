import Image from "next/image";
import Link from "next/link";

import { buttonVariants } from "@/components/ui/button";
import { Card, CardContent } from "@/components/ui/card";
import { portfolioSections, type PortfolioProject } from "@/content/portfolio";
import { cn } from "@/lib/utils";

function DetailGroup({
  title,
  items,
  className,
}: {
  title: string;
  items: string[];
  className?: string;
}) {
  return (
    <div className={cn("flex flex-col rounded-[14px] border border-border bg-background p-4 sm:p-5", className)}>
      <h4 className="text-xs sm:text-sm font-bold text-navy uppercase opacity-80 mb-2 sm:mb-3">
        {title}
      </h4>
      <div className="flex flex-wrap gap-1.5 sm:gap-2">
        {items.map((item) => (
          <span
            key={item}
            className="inline-flex items-center justify-center rounded-full bg-surface-tint/50 border border-border/50 px-2.5 py-1 sm:px-3 sm:py-1.5 font-mono text-xs font-bold text-navy sm:text-xs md:text-sm whitespace-normal text-left"
          >
            {item}
          </span>
        ))}
      </div>
    </div>
  );
}

export function PortfolioProjectCard({ project }: { project: PortfolioProject }) {
  return (
    <article className="group pb-4 sm:pb-0">
      <div className="mb-3 flex flex-wrap items-center justify-between gap-4 sm:mb-6">
        <h3 className="min-w-0 flex-1 truncate text-xl leading-tight font-extrabold text-primary uppercase sm:text-2xl md:text-2xl lg:text-[40px]">
          {project.name}
        </h3>
        <Link
          href={`/case-studies/${project.slug}`}
          className={cn(
            buttonVariants(),
            "h-9 sm:h-10 rounded-full px-4 sm:px-6 font-mono text-xs sm:text-sm uppercase tracking-wide",
          )}
        >
          View Project
        </Link>
      </div>

      <Card className="overflow-hidden border-none bg-background rounded-[20px] shadow-sm transition-all duration-300">
        <CardContent className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6 p-4 sm:p-6 lg:p-6">
          
          {/* Column 1: Image + Description */}
          <div className="flex flex-col gap-4 sm:gap-5 col-span-1">
            <div className="relative aspect-[16/10] sm:aspect-[4/3] w-full overflow-hidden rounded-[20px] sm:rounded-[24px] bg-surface-tint border border-border/50">
              <Image
                src={project.artImage}
                alt={`${project.name} preview`}
                fill
                sizes="(max-width: 768px) 100vw, (max-width: 1024px) 50vw, 400px"
                className="object-cover object-top"
              />
            </div>
            <p className="text-sm leading-relaxed text-body sm:text-base md:text-base lg:text-[17px]">
              {project.description}
            </p>
          </div>

          {/* Column 2: What We Did + Outcome */}
          <div className="flex flex-col gap-4 sm:gap-5 justify-between col-span-1">
            <DetailGroup title="What We Did" items={project.services} className="flex-1" />
            <DetailGroup title="Outcome" items={project.outcomes} className="flex-1" />
          </div>

          {/* Column 3 / Row 2: Detail Image + Category + Timeline + Button */}
          <div className="col-span-1 md:col-span-2 lg:col-span-1 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-1 gap-4 sm:gap-5">
            {/* Detail Image */}
            <div className="relative min-h-[180px] sm:min-h-[220px] w-full overflow-hidden rounded-[14px] border border-border/50">
              {project.summaryBaseImage && (
                <Image
                  src={project.summaryBaseImage}
                  alt={`${project.name} base preview`}
                  fill
                  sizes="(max-width: 768px) 100vw, (max-width: 1024px) 50vw, 300px"
                  className="object-cover"
                />
              )}
              <Image
                src={project.summaryImage}
                alt={`${project.name} detail`}
                fill
                sizes="(max-width: 768px) 100vw, (max-width: 1024px) 50vw, 300px"
                className="object-cover object-bottom"
              />
            </div>

            {/* Category, Timeline & Button */}
            <div className="flex flex-col gap-3 justify-between">
              <div className="rounded-[14px] border border-border bg-surface-tint/30 p-3.5 sm:p-4 md:p-4 lg:p-5">
                <h4 className="text-xs font-bold text-navy uppercase mb-1 opacity-80">
                  Category
                </h4>
                <p className="text-base sm:text-base md:text-base lg:text-lg font-bold text-primary leading-tight">
                  {project.category}
                </p>
              </div>

              <div className="rounded-[14px] border border-border bg-surface-tint/30 p-3.5 sm:p-4 md:p-4 lg:p-5">
                <h4 className="text-xs font-bold text-navy uppercase mb-1 opacity-80">
                  Timeline
                </h4>
                <p className="text-base sm:text-base md:text-base lg:text-lg font-bold text-primary leading-tight">
                  {project.timeline}
                </p>
              </div>
              
              <div className="pt-0.5">
                <Link
                  href="/contact"
                  className={cn(
                    buttonVariants(),
                    "w-full h-10 sm:h-11 md:h-11 lg:h-12 rounded-[16px] sm:rounded-[18px] text-xs sm:text-sm md:text-sm lg:text-base font-bold uppercase tracking-wide"
                  )}
                >
                  Get a free Audit
                </Link>
              </div>
            </div>
          </div>

        </CardContent>
      </Card>
    </article>
  );
}

export function PortfolioContent() {
  return (
    <main className="mx-auto flex w-full max-w-[1416px] flex-col gap-8 px-4 pt-12 sm:pt-8 pb-0 sm:gap-[98px] sm:px-6 md:pt-[70px] sm:pt-[84px] lg:pt-[60px] xl:pt-[94px] sm:pb-12">
      <section className="rounded-[20px] border border-border bg-background p-2 py-12 text-center sm:p-10 md:p-14 lg:p-20 xl:p-[50px]">
        <div className="mx-auto flex max-w-[1316px] flex-col items-center gap-6 sm:gap-8 md:gap-10">
          <h2 className="text-2xl leading-[1.1] font-bold tracking-[0.03em] text-primary uppercase sm:text-5xl lg:text-[56px] xl:text-[72px]">
            Results That Speak Louder Than Promises.
          </h2>
          <p className="text-sm max-w-[935px] leading-relaxed tracking-[0.03em] text-body sm:text-base md:text-lg lg:text-xl font-medium px-6 md:px-10 lg:px-24 xl:px-14">
            Every project is built around one goal helping businesses grow through strategy, creativity, and performance-driven marketing.
          </p>
          <Link
            href="/contact"
            className={cn(
              buttonVariants(),
              "h-11 rounded-full px-6 font-mono text-sm font-semibold sm:h-11 sm:px-6 sm:text-sm md:h-12 md:px-6 md:text-base lg:h-14 lg:px-7 lg:text-base xl:h-16 xl:px-8 xl:text-lg",
            )}
          >
            Let&apos;s Build Yours
          </Link>
        </div>
      </section>

      {portfolioSections.map((section) => (
        <section key={section.title} className="space-y-3 sm:space-y-20 pt-10 sm:pt-8 md:pt-5">
          <div className="flex items-center justify-center sm:justify-between gap-3 min-h-[42px] sm:min-h-[36px] sm:gap-5 mb-6 sm:mb-10 md:mb-16">
            <h2 className="text-2xl leading-tight font-bold text-primary uppercase text-center sm:text-left sm:text-3xl md:text-4xl lg:text-5xl xl:text-[52px]">
              {section.title}
            </h2>
            <Link
              href={section.serviceHref}
              className={cn(
                buttonVariants(),
                "hidden sm:inline-flex h-11 rounded-full px-6 font-mono text-sm font-semibold md:h-12 md:px-6 md:text-base xl:h-16 xl:px-8 xl:text-lg",
              )}
            >
              View Service
            </Link>
          </div>
          <div className="space-y-8 sm:space-y-[98px]">
            {section.projects.map((project) => (
              <PortfolioProjectCard key={project.slug} project={project} />
            ))}
          </div>
        </section>
      ))}
    </main>
  );
}
